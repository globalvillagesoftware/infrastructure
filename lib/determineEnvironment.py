...
from environmentAccess import gvEnvironmentError
...
def LsbWorker() -> None:
    ...
    keys : List[str]\
         = ['LSB Version', 'Distributor ID', 'Description', 'Release',
            'Codename']
    for l in out:  # loop through each line of the command output
        ...
        m : Match = re.fullmatch(
r"""# The regular expression that provides the rules for the basic parsing of a
 ...
 """, l, re.VERBOSE)
        if   m.groups(1) == keys[0]:    # Ignore the LSB Version key
            ...
            if not isinstance(m.group(1), str):
                raise(gvEnvironmentError(
                                         'Syntax error in key describing'
                ...
                raise(gvEnvironmentError(
    f'Syntax error in value of lsb_release text output - got {m.group(2)}'))
